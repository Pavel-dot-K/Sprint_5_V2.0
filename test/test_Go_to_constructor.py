...
def test_login_and_constructor_access_from_account(driver, login, assert_constructor_text):
    ...
    # вызов фикстуры для входа (login встроен как callable)
    login()
    ...
    assert_constructor_text()

# Переход из личного кабинета в конструктор кликом по лого

def test_login_and_constructor_via_logo(driver, login, assert_constructor_text):
    wait = WebDriverWait(driver, 10)
    driver.get("https://stellarburgers.nomoreparties.site/")
